...
im2 = im.crop((280,150,430,300)) #head 오타 ->150x150
...
im2_array = np.asarray(im2) #흑백 이미지
...
im4_array = im4_array * 0.5 # not list -> type => numpy.ndarray

# convert the array back to a unit8 array so we can write to a file
im4_array = im4_array.astype('uint8') # 8 bit
...
grad = np.arange(0,256) #numpy.darray 형태로 0~255값 저장
